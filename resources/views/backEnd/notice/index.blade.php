        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <a href="{{route('notices.create')}}" class="btn btn-primary rounded-pill">Create</a>
                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="button-list">
                                    @if($value->status == 1)
                                    <form method="post" action="{{route('notices.inactive')}}" class="d-inline">
                                    @csrf
                                    ...
                                    @else
                                    <form method="post" action="{{route('notices.active')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
                                    ...
                                    @endif
                                    <a href="{{route('notices.edit',$value->id)}}" class="btn btn-xs btn-primary waves-effect waves-light"><i class="fe-edit-1"></i></a>
                                    <form method="post" action="{{route('notices.destroy')}}" class="d-inline">
                                        @csrf
                                    <input type="hidden" value="{{$value->id}}" name="hidden_id">
