...
@section('title', $childcategory->meta_title ?? $childcategory->childcategoryName)
@push('css')
<link rel="stylesheet" href="{{ asset('frontEnd/css/jquery-ui.css') }}" />
<style>
/* ───── Childcategory Page Styles (Matching Category Page) ───── */
    ...
    <div class="container">
        <div class="pdp-header-inner">
            <div class="pdp-breadcrumb">
                ...
                @if($childcategory->subcategory && $childcategory->subcategory->category)
                <span>›</span>
                <a href="{{ route('category', $childcategory->subcategory->category->slug) }}">{{ $childcategory->subcategory->category->name }}</a>
                @endif
                @if($childcategory->subcategory)
                <span>›</span>
                <a href="{{ url('subcategory/'.$childcategory->subcategory->slug) }}">{{ $childcategory->subcategory->subcategoryName }}</a>
                @endif
                <span>›</span>
        ...
        <div class="cat-toolbar">
            <div class="cat-toolbar-left">
                ...
                <span class="showing-data">
                    ...
                    of <strong>{{ number_format($products->total()) }}</strong> results for <strong>{{ $childcategory->childcategoryName }}</strong>
                </span>
            </div>
        ...
        <div class="row">
                        ...
                        <div class="cat-sidebar-body" id="catSidebarCategories">
                            <ul class="cat-list">
                                ...
                                <li class="cat-item">
                                    ...
                                    @if($subs->count() > 0)
                                    <button class="cat-toggle {{ $subs->contains('id', $childcategory->subcategory_id) ? 'active' : '' }}" data-toggle="subcat" data-target="#catSub-{{ $cat->id }}">
                                        <span>{{ $cat->name }}</span>
                                        <i class="fa fa-chevron-right"></i>
                                    </button>
                                    <ul class="cat-sub-list {{ $subs->contains('id', $childcategory->subcategory_id) ? 'open' : '' }}" id="catSub-{{ $cat->id }}">
                                        ...
                                        @foreach($subs as $sub)
                                        <li class="cat-sub-item">
                                            <a href="{{ url('subcategory/'.$sub->slug) }}" class="cat-sub-link {{ $sub->id == $childcategory->subcategory_id ? 'active' : '' }}">{{ $sub->subcategoryName }}</a>
                                        </li>
                                        @endforeach
                                    </ul>
                                    @else
                            ...
                            <div class="cat-price-range">
                                <form action="" method="GET" id="catPriceForm">
                                    ...
                                    <div class="price-filter-actions">
                                        ...
                                        <a href="{{ route('products', $childcategory->slug) }}" class="btn-price-reset">Reset</a>
                                        @endif
                                    </div>
                                </form>
                            </div>
            ...
            <div class="col-lg-9">
                ...
                <div class="cat-empty">
                    ...
                    <a href="{{ route('products', $childcategory->slug) }}" class="btn-price-filter" style="display:inline-block;padding:10px 30px;text-decoration:none;">View All</a>
                </div>
                @endif
            </div>
        </div>
