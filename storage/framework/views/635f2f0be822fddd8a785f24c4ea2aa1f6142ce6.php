...
<div class="container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <form action="<?php echo e(route('admin.order.update')); ?>" method="POST" class="pos_form" data-parsley-validate="" enctype="multipart/form-data">
                        <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>">
                          ...
                          <table class="table table-bordered" style="margin-bottom:0;">
                            ...
                            <tbody id="cartTable">
                              ...
                              <tr>
                                <td><img height="30" src="<?php echo e(asset($value->options->image)); ?>" style="border-radius:4px;object-fit:cover;width:40px;"></td>
                                <td style="font-size:12px;"><?php echo e($value->name); ?></td>
                                <td style="font-size:11px;">
                                  <?php if($value->options->product_color || $value->options->product_size): ?>
                                    <span class="badge bg-info" style="font-size:10px;"><?php echo e($value->options->product_color ?: ''); ?><?php echo e($value->options->product_color && $value->options->product_size ? ' / ' : ''); ?><?php echo e($value->options->product_size ?: ''); ?></span>
                                  <?php else: ?>
                                ...
                                <td style="font-size:12px;"><?php echo e($value->price); ?></td>
                                <td><input type="number" class="form-control form-control-sm product_discount" style="width:55px;" value="<?php echo e($value->options->product_discount); ?>" placeholder="0" data-id="<?php echo e($value->rowId); ?>"></td>
                                <td style="font-size:12px;"><?php echo e(($value->price - $value->options->product_discount)*$value->qty); ?></td>
                        ...
                        <div class="col-sm-5">
                          <div class="card bg-light mb-3" style="border:1px solid #dee2e6;">
                           <div class="card-body py-3 px-3">
                            ...
                            <div class="mb-2"><input type="text" class="form-control form-control-sm" placeholder="Customer Name" name="name" value="<?php echo e($shippinginfo->name); ?>" required></div>
                            <div class="mb-2"><input type="number" class="form-control form-control-sm" placeholder="Phone Number" name="phone" value="<?php echo e($shippinginfo->phone); ?>" required></div>
                            <div class="mb-2"><input type="text" placeholder="Address" class="form-control form-control-sm" name="address" value="<?php echo e($shippinginfo->address); ?>" required></div>
                            <div class="mb-2">
                              <select class="form-control form-control-sm" name="area" required>
                               <option value="">Delivery Area</option>
                               <?php $__currentLoopData = $shippingcharge; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                               <option value="<?php echo e($sc->id); ?>" <?php if($shippinginfo->area == $sc->name): ?> selected <?php endif; ?>><?php echo e($sc->name); ?></option>
                              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                              </select>
                            </div>
                            <div class="row g-1">
                             <div class="col-6"><label style="font-size:10px;color:#888;">Order Date</label><input type="date" name="order_date" class="form-control form-control-sm" value="<?php echo e($order->order_date ? $order->order_date->format('Y-m-d') : date('Y-m-d')); ?>"></div>
                             <div class="col-6"><label style="font-size:10px;color:#888;">Delivery Date</label><input type="date" name="delivery_date" class="form-control form-control-sm" value="<?php echo e($order->delivery_date ? $order->delivery_date->format('Y-m-d') : ''); ?>"></div>
                            </div>
                           </div>
                          </div>
                           ...
                           <div class="card-body py-3 px-3">
                            ...
                            <table class="table table-sm table-borderless mb-2" style="font-size:13px;">
                             <tbody id="cart_details">
                              ...
                              ?>
                              <tr><td>Sub Total</td><td class="text-end"><?php echo e($subtotal); ?></td></tr>
                              <tr><td>Shipping Fee</td><td class="text-end"><?php echo e($shipping); ?></td></tr>
                              <tr><td>Discount</td><td class="text-end">-<?php echo e($total_discount); ?></td></tr>
                              <tr class="fw-bold" style="border-top:2px solid #dee2e6;"><td>Total</td><td class="text-end"><?php echo e(($subtotal + $shipping)- $total_discount); ?></td></tr>
                             </tbody>
                            </table>
                            <div class="text-end">
    ...
    function cart_content(){
      ...
      $('#cart_add').on('change', function() {
        ...
        $.ajax({
          ...
          url: "<?php echo e(route('admin.order.product_variants')); ?>",
          dataType:"json",
          success: function(p) {
            ...
            if (hasColor || hasSize) {
              ...
              $('#variantProductImage').attr('src', p.image || '<?php echo e(asset("backEnd/assets/images/default-product.png")); ?>');
      ...
      $(document).on('click', '.cart_increment', function(e) {
        ...
        $.ajax({ cache:false, data:{id:id, qty:qty}, type:"GET", url:"<?php echo e(route('admin.order.cart_increment')); ?>", dataType:"json", success: function(){ refreshCart(); } });
      });
      $(document).on('click', '.cart_decrement', function(e) {
        e.preventDefault();
        var id = $(this).data('id'), qty = $(this).closest('tr').find('input').val();
        $.ajax({ cache:false, data:{id:id, qty:qty}, type:"GET", url:"<?php echo e(route('admin.order.cart_decrement')); ?>", dataType:"json", success: function(){ refreshCart(); } });
      });
      $(document).on('click', '.cart_remove', function(e) {
        e.preventDefault();
        var id = $(this).data('id');
        $.ajax({ cache:false, data:{id:id}, type:"GET", url:"<?php echo e(route('admin.order.cart_remove')); ?>", dataType:"json", success: function(){ refreshCart(); } });
      });
      $(document).on('change', '.product_discount', function() {
        var id = $(this).data('id'), discount = $(this).val();
        $.ajax({ cache:false, data:{id:id, discount:discount}, type:"GET", url:"<?php echo e(route('admin.order.product_discount')); ?>", dataType:"json", success: function(){ refreshCart(); } });
      });
      $(".cartclear").click(function(e) {
        $.ajax({ cache:false, type:"GET", url:"<?php echo e(route('admin.order.cart_clear')); ?>", dataType:"json", success: function(){ refreshCart(); } });
      });
      $("#area").on("change", function () {
        var id = $(this).val();
        $.ajax({ type:"GET", data:{ id: id }, url:"<?php echo e(route('admin.order.cart_shipping')); ?>", dataType:"html", success: function(){ refreshCart(); } });
      });
</script>
<?php $__env->stopSection(); ?>
...
<?php echo $__env->make('backEnd.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/smgiyasuddin/ecommerce/ganienterprise/resources/views/backEnd/order/edit.blade.php ENDPATH**/ ?>