...
unset($__errorArgs, $__bag); ?>">
          ...
          <option value="<?php echo e($value->id); ?>"><?php echo e($value->name); ?> (<?php echo e($value->product_code); ?>)</option>
          ...
          <tr>
           ...
           <td>
            <?php if($value->options->product_color || $value->options->product_size): ?>
             <span class="badge bg-info badge-variant"><?php echo e($value->options->product_color ?: ''); ?><?php echo e($value->options->product_color && $value->options->product_size ? ' / ' : ''); ?><?php echo e($value->options->product_size ?: ''); ?></span>
            <?php else: ?>
           ...
           <td><input type="number" class="form-control form-control-sm product_discount" style="width:70px;" value="<?php echo e($value->options->product_discount); ?>" placeholder="0" data-id="<?php echo e($value->rowId); ?>" /></td>
         ...
         <div class="row g-2">
          <div class="col-sm-12">
           ...
           <select id="customer_select" class="form-control form-control-sm select2">
            ...
            <option value="<?php echo e($customer->id); ?>" data-phone="<?php echo e($customer->phone); ?>" data-address="<?php echo e($customer->address); ?>" data-area="<?php echo e($customer->area); ?>">
          ...
          <div class="col-sm-6">
           ...
           <input type="date" name="order_date" class="form-control form-control-sm" value="<?php echo e(date('Y-m-d')); ?>">
          </div>
          <div class="col-sm-6">
        ...
        <div class="card-body py-3">
         ...
         <table class="table table-sm table-borderless mb-0">
          <tbody id="cart_details">
           <?php $subtotal = Cart::instance('pos_shopping')->subtotal(); $subtotal = str_replace(',','',$subtotal); $subtotal = str_replace('.00', '',$subtotal); $shipping = Session::get('pos_shipping'); $total_discount = Session::get('pos_discount')+Session::get('product_discount'); ?>
           <tr><td>Subtotal</td><td class="text-end">৳<?php echo e($subtotal); ?></td></tr>
           <tr><td>Shipping</td><td class="text-end">৳<?php echo e($shipping ?? '0'); ?></td></tr>
           <tr><td>Discount</td><td class="text-end">-৳<?php echo e($total_discount); ?></td></tr>
           <tr class="fw-bold"><td>Total</td><td class="text-end">৳<?php echo e(($subtotal + ($shipping ?? 0)) - $total_discount); ?></td></tr>
          </tbody>
         </table>
        </div>
       </div>
...
$(document).ready(function () {
 ...
 function cart_content() {
  $.ajax({ type:"GET", url:"<?php echo e(route('admin.order.cart_content')); ?>", dataType:"html",
   success: function(r){ $("#cartTable").html(r); }
  });
 }
 function cart_details() {
  $.ajax({ type:"GET", url:"<?php echo e(route('admin.order.cart_details')); ?>", dataType:"html",
 ...
 $(document).on('click', '.cart_increment', function(e) {
  ...
  $.ajax({ cache:false, data:{id:id, qty:qty}, type:"GET", url:"<?php echo e(route('admin.order.cart_increment')); ?>", dataType:"json",
 ...
 $(document).on('click', '.cart_decrement', function(e) {
  e.preventDefault();
  var id = $(this).data('id'), qty = $(this).closest('tr').find('input').val();
  $.ajax({ cache:false, data:{id:id, qty:qty}, type:"GET", url:"<?php echo e(route('admin.order.cart_decrement')); ?>", dataType:"json",
   success: function(){ refreshCart(); }
  });
 });
 $(document).on('click', '.cart_remove', function(e) {
  e.preventDefault();
  var id = $(this).data('id');
  $.ajax({ cache:false, data:{id:id}, type:"GET", url:"<?php echo e(route('admin.order.cart_remove')); ?>", dataType:"json",
   success: function(){ refreshCart(); }
  });
 });
 $(document).on('change', '.product_discount', function() {
  var id = $(this).data('id'), discount = $(this).val();
  $.ajax({ cache:false, data:{id:id, discount:discount}, type:"GET", url:"<?php echo e(route('admin.order.product_discount')); ?>", dataType:"json",
   success: function(){ refreshCart(); }
  });
 });
 $("#area").on("change", function () {
  var id = $(this).val();
  $.ajax({ type:"GET", data:{ id: id }, url:"<?php echo e(route('admin.order.cart_shipping')); ?>", dataType:"html",
   success: function(){ refreshCart(); }
  });
 });
 $(".cartclear").click(function(e) {
  $.ajax({ cache:false, type:"GET", url:"<?php echo e(route('admin.order.cart_clear')); ?>", dataType:"json",
   success: function(){ refreshCart(); }
  });
 });
});
...
<?php echo $__env->make('backEnd.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/smgiyasuddin/ecommerce/ganienterprise/resources/views/backEnd/order/create.blade.php ENDPATH**/ ?>