...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->proSize){
            $sizeData = [];
            foreach($request->proSize as $sizeId){
                $sizeData[$sizeId] = [
                    'price' => $request->sizePrice[$sizeId] ?? null,
        ...
        if($request->proColor){
            $colorData = [];
            foreach($request->proColor as $colorId){
                $colorData[$colorId] = [
                    'price' => $request->colorPrice[$colorId] ?? null,
    ...
    {
        $edit_data = Product::with('images', 'mainImages')->find($id);
        ...
        $selectcolors = Productcolor::where('product_id',$id)->with('color')->get();
        $selectsizes = Productsize::where('product_id',$id)->with('size')->get();
    ...
    public function update(Request $request)
    {
       $this->validate($request, [
        ...
        if($request->proSize){
            ...
            $update_data->sizes()->sync($sizeData);
        } else {
            $update_data->sizes()->detach();
        }

        // Sync color variants with price and stock
        if($request->proColor){
            $colorData = [];
            foreach($request->proColor as $colorId){
                $colorData[$colorId] = [
            ...
            $update_data->colors()->sync($colorData);
        } else {
            $update_data->colors()->detach();
        }
        ...
        // Save/update color-specific images
        if($request->hasFile('colorImage')){
            foreach($request->file('colorImage') as $colorId => $image){
                if($image){
                    // Delete existing color image for this product+color combination
                    $existingImage = Productimage::where('product_id', $update_data->id)
